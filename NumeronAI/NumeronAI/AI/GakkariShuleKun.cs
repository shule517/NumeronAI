...
{
	class GakkariShuleKun : INumeronAI
	{
		/// <summary>
		/// ランダム生成クラス
		/// </summary>
		...
		List<int> INumeronAI.Answer()
		{
			...
			for (int i = 0; i < 1000; i++)
			{
				...
				float weight = 0.7f;
				p.Point += ((point[p.Number[0]] * pointWeight) + (pointDigit1[p.Number[0]] * digitWeight));// *((point[p.Number[0]] == 0) ? weight : 0);
				p.Point += ((point[p.Number[1]] * pointWeight) + (pointDigit2[p.Number[1]] * digitWeight));// *((point[p.Number[1]] == 0) ? weight : 0);
				p.Point += ((point[p.Number[2]] * pointWeight) + (pointDigit3[p.Number[2]] * digitWeight));// *((point[p.Number[2]] == 0) ? weight : 0);
